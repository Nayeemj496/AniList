...
const adminAddAnimeControllerPOST = async (req, res) => {
    ...
    if(req.session.user) {
        ...
        const maxanimeid = await connection.execute(`SELECT MAX(ANIME_ID) FROM ANIME`)
...
const adminAddMangaControllerPOST = async (req, res) => {
  ...
  if(req.session.user) {
    ...
    const maxmangaid = await connection.execute(`SELECT MAX(MANGA_ID) FROM MANGA`)
...
const adminUpdateAnimeCharacterControllerPOST = async(req,res)=>{
    ...
    if(req.session.user){
        ...
        if(existingcharacter.rows.length > 0) {
            ...
            const alreadyentry = await connection.execute(`SELECT * FROM HAS WHERE CHARACTER_ID = :charid`,{
                charid: idgot
            })
            if(alreadyentry.rows.length > 0){
                ...
                const searchanime = await connection.execute(`SELECT ANIME_ID FROM ANIME WHERE ENGLISH = :english`,{
                    english: req.body.animename
                })
                const animeid = searchanime.rows[0][0]
                await connection.execute(`INSERT INTO HAS(ANIME_ID,CHARACTER_ID) VALUES(:animeid,:charid)`,{
                    animeid: animeid,
                    charid: idgot
                })
                await connection.commit()
            ...
            const maxcharid = await connection.execute(`SELECT MAX(CHARACTER_ID) FROM CHARACTER`)
...
const adminUpdateAnimeStaffControllerPOST = async(req,res)=>{
    ...
    if(req.session.user){
        ...
        if(existingstaff.rows.length > 0){
            const dbanimeid = await connection.execute(`SELECT ANIME_ID FROM ANIME WHERE ENGLISH = :english`,{
                english: req.body.animename
            })
            const animeid = dbanimeid.rows[0][0]
            await connection.execute(`INSERT INTO ANIME_STAFF(ANIME_ID,STAFF_ID) VALUES(:animeid,:staffid)`,{
                animeid: animeid,
            ...
            await connection.execute(sqlforinsertingstaff,{
                ...
                yearsactivestart: isNaN(req.body.yearsactivestart) ? null : req.body.yearsactivestart,
                yearsactiveend: isNaN(req.body.yearsactiveend) ? null : req.body.yearsactiveend,
                hometown: req.body.hometown,
                role: req.body.role,
                gender:req.body.gender,
                description: req.body.description
...
const adminUpdateMangaCharacterControllerGET = async(req,res)=>{
    console.log("in the adminUpdateMangaCharacterControllerGET");
    console.log(req.url, req.method);
    if(req.session.user) {
      res.render("admin_add_manga_character", {
          isAdmin: req.session.user.ROLE === "ADMIN" ? true : false,
          userimage: req.session.user.USER_IMAGE || "/images/photos/user.png",
          username: req.session.user.USERNAME
...
const adminUpdateMangaCharacterControllerPOST = async(req,res)=>{
    console.log("in the adminUpdateMangaCharacterControllerPOST");
    console.log(req.url, req.method);
    if(req.session.user){
        const connection = await connect()
        const existingcharacter = await connection.execute(`SELECT * FROM CHARACTER WHERE FULL_NAME = :fullname`,{
            fullname: req.body.fullname
        })
...
const adminUpdateMangaStaffControllerGET = async(req,res)=>{
    console.log("in the adminUpdateMangaStaffControllerGET");
  console.log(req.url, req.method);
  if(req.session.user) {
    res.render("admin_add_manga_staff", {
        isAdmin: req.session.user.ROLE === "ADMIN" ? true : false,
        userimage: req.session.user.USER_IMAGE || "/images/photos/user.png",
        username: req.session.user.USERNAME
...
const adminUpdateMangaStaffControllerPOST = async(req,res)=>{
    console.log('in the adminUpdateMangaStaffControllerPOST')
    console.log(req.url, req.method)
    if(req.session.user){
        const connection = await connect()
        const existingstaff = await connection.execute(
        ...
        if(existingstaff.rows.length > 0){
            const existingstaffid = existingstaff.rows[0][0]
            const checkmangastaff = await connection.execute(`SELECT * FROM MANGA_STAFF WHERE STAFF_ID = :staffid`,{
                staffid: existingstaffid
            })
            if(checkmangastaff.rows.length > 0){
                res.redirect("/admin")
            }
            else{
                const dbmangaid = await connection.execute(`SELECT MANGA_ID FROM MANGA WHERE ENGLISH = :english`,{
                    english: req.body.manganame
                })
                const mangaid = dbmangaid.rows[0][0]
                await connection.execute(`INSERT INTO MANGA_STAFF(MANGA_ID,STAFF_ID) VALUES(:mangaid,:staffid)`,{
                    mangaid: mangaid,
                    staffid: existingstaffid
                })
                await connection.commit()
  ...
  if(req.session.user && req.session.user.ROLE === "ADMIN") {
    ...
    if (obj.hasOwnProperty("update")) {
        ...
        res.json({
          ...
          const utitle = obj.selectManga 
          console.log(utitle)
          const id = await connection.execute(`SELECT MANGA_ID FROM MANGA WHERE ENGLISH = :utitle`,{
                utitle: utitle
            
          })
          await connection.execute(`UPDATE CHARACTER SET MANGA_ID = NULL WHERE MANGA_ID = :id`,{
                id: id.rows[0][0]
          })
          await connection.execute(
...
module.exports = {
    ...
    adminUpdateMangaCharacterControllerGET,
    adminUpdateMangaCharacterControllerPOST,
    adminUpdateMangaStaffControllerGET,
    adminUpdateMangaStaffControllerPOST,  
